...
double TeosBase::gsw_deltasa_atlas(double p,double lon,double lat)
{
	...
	double l_r1, s1, l_t1, ndepth_max;
   double return_value;
   ...
   else
   {
		...
		else
		{
			...
			*************************************************/
			...
			l_r1	= (lon-pSaarData->get_longs_ref(indx0))/
			...
			l_t1	= (p-pSaarData->get_p_ref(indz0))/
			...
			sa_upper	= (1.0-s1)*(dsar[0] + l_r1*(dsar[1]-dsar[0])) +
						  s1*(dsar[3] + l_r1*(dsar[2]-dsar[3]));
			...
			sa_lower	= (1.0-s1)*(dsar[0] + l_r1*(dsar[1]-dsar[0])) +
						  s1*(dsar[3] + l_r1*(dsar[2]-dsar[3]));
			...
			return_value = sa_upper + l_t1*(sa_lower-sa_upper);
...
{
	double sa, l_saar = gsw_saar(p,lon,lat);

	if (l_saar >= cppGSW_ERROR_LIMIT)
	{
		sa  = cppGSW_INVALID_VALUE;
	}
	else
	{
		sa  = ((1.0 + 0.35)*l_saar)/(1.0 - 0.35*l_saar);
	}
...
%==========================================================================
***************************************************************************/
double TeosBase::gsw_pt_from_t(double sa, double t, double p, double lp_ref)
{
	/**  for GSW_TEOS10_CONSTANTS use gtc */
	...
	{
		pt_old	= pt;
		dentropy	= gsw_entropy_part(sa,pt_old,lp_ref) - true_entropy_part;
		pt = pt_old - dentropy/dentropy_dt;
		ptm = 0.5*(pt + pt_old);
		dentropy_dt	= -gsw_gibbs(0,2,0,sa,ptm,lp_ref);
		pt = pt_old - dentropy/dentropy_dt;
	}
...
double TeosBase::gsw_sa_from_sp(double sp, double p, double lon, double lat)
{
	...
	double return_value, lsaar, gsw_sa_baltic;
	...
	{
	   lsaar = gsw_saar(p,lon,lat);

	   if (lsaar == cppGSW_INVALID_VALUE)
		{
		   return_value = lsaar;
      }
      else
	   {
	      return_value = (gtc.gsw_ups*sp*(1.0 + lsaar));
      }
	}
...
{
	double return_value = cppGSW_INVALID_VALUE, lsaar = gsw_saar(p,lon,lat);
	...
	if (lsaar != cppGSW_INVALID_VALUE)
	{
	   ...
	   */
	   return_value = (sstar*(1.0 + lsaar)/(1.0 - 0.35e0* lsaar));
	}
...
double TeosBase::gsw_saar(double p, double lon, double lat)
{
	...
	int nx=gsw_nx, ny=gsw_ny, nz=gsw_nz;
	int indx0, indy0, indz0, k, ndepth_index;
	double lsaar[4], saar_old[4];
	double sa_upper, sa_lower, dlong, dlat;
	double lr1, s1, lt1, ndepth_max, return_value;
	...
	else
   {
      ...
      else
      {
			...
			lr1 = (lon-pSaarData->get_longs_ref(indx0))/
			...
			lt1 = (p-pSaarData->get_p_ref(indz0))/
			...
			for (k=0; k<4; k++)
			{
				...
				lsaar[k] = pSaarData->get_saar_ref(kpos);
			}

			if (gsaar.longs_pan[0] <= lon && lon <= gsaar.longs_pan[gsaar.npan-1]-0.001 &&
					gsaar.lats_pan[gsaar.npan-1] <= lat && lat <= gsaar.lats_pan[0])
			{
				memmove(saar_old, lsaar,4*sizeof (double));
				gsw_add_barrier(saar_old,lon,lat,pSaarData->get_longs_ref(indx0),
									 pSaarData->get_lats_ref(indy0),dlong,dlat, lsaar);
			}
			else if (fabs(gsw_sum(lsaar, sizeof (lsaar)/sizeof (double)))
			            >= cppGSW_ERROR_LIMIT)
			{
				memmove(saar_old, lsaar,4*sizeof (double));
				gsw_add_mean(saar_old, lsaar);
			}

			sa_upper = (1.0-s1)*(lsaar[0] + lr1*(lsaar[1]- lsaar[0])) +
						  s1*(lsaar[3] + lr1*(lsaar[2]- lsaar[3]));
			...
			sa_lower = (1.0-s1)*(lsaar[0] + lr1*(lsaar[1]- lsaar[0])) +
						  s1*(lsaar[3] + lr1*(lsaar[2]- lsaar[3]));
			...
			return_value = sa_upper + lt1*(sa_lower-sa_upper);
...
double TeosBase::gsw_sp_from_sa(double sa, double p, double lon, double lat)
{
	...
	double return_value, lsaar, sp_baltic;
   ...
   else
   {
	   ...
	   {
	      return_value = (sa/gtc.gsw_ups) / (1.0 + lsaar);
      }
	}
...
double TeosBase::gsw_sstar_from_sa(double sa,double p,double lon,double lat)
{
	...
	*/
	if (lsaar != cppGSW_INVALID_VALUE)
	{
		return_value = sa*(1.0 - 0.35*lsaar) / 1.0 + lsaar;
	}
...
double TeosBase::gsw_sstar_from_sp(double sp,double p,double lon,double lat)
{
	...
	double return_value, lsaar, sstar_baltic;
   ...
   else
   {
	   ...
	   {
	      return_value = gtc.gsw_ups * sp * (1.0 - 0.35 * lsaar);
	   }
	}
...
double TeosBase::gsw_R_from_SP(double SP, double t, double p)
{
   /** for GSW_SP_COEFFICIENTS use gspc */
	...
	double lr1 =   1.672940491817403e-1;
	...
	double lr20 =  1.122200786423241e-12;
   ...
   if (SP >= 0.003 && SP < 0.25)
   {
	   Rtx =  r0 + x *(lr1 + r4 * t68
                  ...
                  + x *(r13 + x * r18) + t68 * (r14 + r19 *x + lr20 * t68))));
   }
